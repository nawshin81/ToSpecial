...
if (isset($_REQUEST['id'])) {
	...
	if (isset($_POST['postComment' . $getid . ''])) {
		...
		$query = mysqli_query($conn,"SELECT id,added_by  FROM posts WHERE id='$getid'");
		$query_row = mysqli_fetch_assoc($query);
		$posted_to = $query_row['added_by'];
		$insertPost = mysqli_query($conn,"INSERT INTO post_comments VALUES ('','$post_body','$date_added', NOW(), '$user','$posted_to','no','$getid')");
	}
	}
	//post query
	$query = mysqli_query($conn,"SELECT id,added_by  FROM posts WHERE id='$getid'");
	...
	$postby_query = mysqli_query($conn,"SELECT * FROM users WHERE username='$posted_to'");
	$postby_row = mysqli_fetch_assoc($postby_query);
	//getting user gender
	...
	<?php 
		if ($postby_query) {
			if (($posted_to == $user) || ($user_query)) {
				echo "
				<div style='margin: 0 52px;'>
				<form action='comment_frame.php?id=".$getid."' method='POST' name='postComment".$getid."'>
					<input style='padding: 10px 3px; width: 83%; margin: 0 0 5px 0; resize: none; border: 1px solid #505050;' name='post_body' placeholder= 'Leave your comment here!'>
	...
	$get_comments = mysqli_query($conn,"SELECT * FROM post_comments WHERE post_id='$getid' ORDER BY id DESC LIMIT 3");
	...
	while ($comment = mysqli_fetch_assoc($get_comments)) {
		...
		$get_user_info = mysqli_query($conn,"SELECT * FROM users WHERE username='$post_by'");
		...
		//check for propic delete
						$pro_changed = mysqli_query($conn,"SELECT * FROM posts WHERE added_by='$post_by' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
