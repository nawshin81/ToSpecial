...
$get_info = mysqli_query($conn, "SELECT country,city,hometown FROM users WHERE username='$user'");
...
if ($send) {
	...
	$city = trim($city);
	$city = ($city);//mysql_real_escape_string
	$city = ucwords($city);
	$hometown = strip_tags(@$_POST['hometown']);
	$hometown = trim($hometown);
	$hometown = ($hometown);//mysql_real_escape_string
	$hometown = ucwords($hometown);
		//submit the form to database
		$info_submit_query = mysqli_query($conn, "UPDATE users SET city='$city' WHERE username='$user'");
		$info_submit_query = mysqli_query($conn, "UPDATE users SET country='$country' WHERE username='$user'");
		$info_submit_query = mysqli_query($conn, "UPDATE users SET hometown='$hometown' WHERE username='$user'");
		...
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					...
					<li><a href="location_update.php" style=" background-color: #e76f39; border-radius: 4px; color: #fff;">Location and Places</a></li>
