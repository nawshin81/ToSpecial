...
$username = ($username);//mysqli_real_escape_string

//Check whether the user has uploaded a profile pic or not
$check_pic = mysqli_query($conn,"SELECT profile_pic, first_name, username FROM users WHERE username='$username' || email='$username'");
$get_pic_row = mysqli_fetch_assoc($check_pic);
$profile_pic_db = $get_pic_row['profile_pic'];
$pic_uname = $get_pic_row['username'];
$fullname_db = $get_pic_row['first_name'];
//check for propic delete
$pro_changed = mysqli_query($conn,"SELECT * FROM posts WHERE added_by='$pic_uname' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
...
if (isset($_POST['searchId'])) {
	...
	$username_query = mysqli_query($conn,"SELECT * FROM users WHERE username='$username' || email='$username'");
		...
		if ($get_block_fetch_query == 1 ) {
			...
			$succes_echoEmail = "
				...
				<form action='passRecover.php' method='POST'>
					<span style='font-size:  margin: 13px 0 0 13px;  font-weight: 800; color: #e76f39'>".$fullname_db."</span></br>
...
$recovEmail = ($recovEmail);   //mysqli_real_escape_string
if (isset($_POST['searchRecovEmail'])) {
	$username = $_SESSION['username'];
	$searchRecovEmail_query = mysqli_query($conn,"SELECT * FROM users WHERE (email='$recovEmail' AND username='$username')");
	...
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($conn,$confirmCodeQuery)) {
			...
			";
			//mail($recovEmail,"ToSpecial Confirmation Code",$msg, "From:ToSpecial <no-reply@ToSpecial.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
...
if (isset($_POST['searchRecovEmail2'])) {
	...
	$searchRecovEmail_query = mysqli_query($conn,"SELECT * FROM users WHERE (email='$user_con_email' AND username='$user_con_name')");
	$searchRecovEmail_query_num = mysqli_num_rows($searchRecovEmail_query);
	$usernm_fetch_query = mysqli_fetch_assoc($searchRecovEmail_query);
	$get_first_nm_fetch_query = $usernm_fetch_query['first_name'];
	if ($searchRecovEmail_query_num >= 1) {
		...
		if (mysqli_query($conn,$confirmCodeQuery)) {
			...
			N.B. If it's not by you, delete this mail or forget it.
			";
			mail($user_con_email,"ToSpecial Confirmation Code",$msg, "From:ToSpecial <no-reply@ToSpecial.com>");
			$success_email_cnfrm = "
				<form action='passRecover.php' method='POST'>
						Verification Code:</br></br><input type='text' name='confirmMailCode' class='placeholder' size='30' required autofocus></br></br>
					</hr>
...
$confirmMailCode = ($confirmMailCode);  //mysqli_real_escape_string
$newpassword = ($newpassword); //mysqli_real_escape_string
$repear_password = ($repear_password);//mysqli_real_escape_string
if (isset($_POST['confrmRessetpass'])) {
	$unameCnfrm = $_SESSION['final_uname'];
	if ($unameCnfrm != '') {
		$unameCnfrm_query =mysqli_query($conn,"SELECT * FROM users WHERE username='$unameCnfrm'");
			...
			if ($confirmMailCode == $get_confirmCode_fetch_query) {
				if ($newpassword == $repear_password) {
					...
					if (mysqli_query($conn,$updatePassQuery)) {
						...
						// send email
						$msg = "Successfully your ToSpecial password has been changed. 
						
						If it's not by you, please follow the link.
				
						http://www.ToSpecial.com/confirmationPass.php?u=".$get_uName_fetch_query."&code=".$get_confirmCode_fetch_query ."
						
						";
						//mail($get_uEmail_fetch_query ,"ToSpecial Password Changed",$msg, "From:ToSpecial <no-reply@ToSpecial.com>");
					}
				}else {
	...
	<div>
		<div class="headerLogin">
			<div class="login_menubar clearfix">
				<div class="menu_logo">
					<h1>
						<a title="Go to Home" href="r_home.php">
							<b>ToSpecial</b>
						</a>
					</h1>
				</div>
						...
						<table class="menu_login_container">
							<tr class="login_">
								...
								<?php
									...
									echo '
									<a href="signin.php"><input type="submit" name="login" class="uiloginbutton" style="margin-top: 17px;" value="Sign Up"></a>
									';
									}
		...
		<div class="pass_body2">
				<?php
					if (isset($success_msg)) {
						...
						echo '
							<form action="passRecover.php" method="POST">
								<p style="color: #e76f39">Find your Account!</p></br>
									Enter email or username:</br></br><input type="text" name="username" class="placeholder" size="30" required autofocus></br></br>
								</hr></br>
								<input style="background-color: #e76f39;"class="submRecov" type="submit" name="searchId" id="senddata" value="Search">
							</form>
						';
					}
				?>
