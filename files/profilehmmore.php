...
if (!isset($_SESSION['user_login'])) {
 ...
 if ($profilehmlastid >= 1) {
 			//getting username
		 $result = mysqli_query($conn,"SELECT * FROM posts WHERE id ='$profilehmlastid'") or die(mysqli_error());
		 $name = mysqli_fetch_assoc($result);
		 $profilehm_uname = $name['user_posted_to'];
		//timeline query table
		$getposts = mysqli_query($conn,"SELECT * FROM posts WHERE user_posted_to ='$profilehm_uname' AND note='0' AND report='0' AND id < $profilehmlastid ORDER BY id DESC LIMIT 12") or die(mysqli_error());
