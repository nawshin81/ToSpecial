...
<?php 
	...
	$about_query = mysqli_query($conn,"SELECT first_name,verify_id,user_type FROM users WHERE username='$username'");
	$get_result = mysqli_fetch_assoc($about_query);
	$first_name_user = $get_result['first_name'];
	$verify_id_user = $get_result['verify_id'];
	$pro_type = $get_result['user_type'];
						...
						<ul style="line-height: 1.3;">
							<?php 
								if ($verify_id_user == 'yes') {
									echo '<span style="font-size: 25px; margin-right: 8px; float: left; font-weight: 800; color: #ffffff">'.$Stype.$first_name_user.'</span><div class="verifiedicon" style="background: url(img/verifiLogo.png) repeat; background-size: cover !important;" title="Verified profile"></div>';
								}else {
									echo '<span style="font-size: 25px; margin-right: 13px; float: left; font-weight: 800; color: #ffffff">'.$Stype.$first_name_user.'</span>';
								}
							 ?>
