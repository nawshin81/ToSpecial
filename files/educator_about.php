...
if ($num == 1) {
	...
	echo '<li style="float: right;">			
					<div >
						...
						<li><a href="educator_about.php?u='.$username.'" style="background-color: #cdcdcd; color: #505050">About</a></li>
	...
	$about_query = mysqli_query($conn,"SELECT educator.work_in,educator.work_as,educator.academic,users.city,users.hometown,users.queote,users.bio,users.mobile,users.pub_email FROM users,educator WHERE users.username='$username' and educator.username='$username'");
					$get_result = mysqli_fetch_assoc($about_query);
					$school_name_user = $get_result['academic'];
					$city_name_user = $get_result['city'];
					$hometown_name_user = $get_result['hometown'];
					$user_queote = $get_result['queote'];
					$user_bio = $get_result['bio'];
					$user_company = $get_result['work_in'];
					$user_position = $get_result['work_as'];
	...
	echo '<div class="uiaccountstyle" style="text-align: left; height: auto; width: 507px; padding: 25px; margin: 15px auto;">
		<div>
			...
			<hr style="background-color: #ddd;">';
					...
					echo '
					<p style="font-size: 15px; margin-left: 25px; font-weight: bold; color: #505050;">'.$user_company.'<br></p>
			<p style=" font-weight: bold; margin-left: 25px; ">'.$user_position.'<br></p></br>
			<p style="font-size: 15px; margin-left: 25px; font-weight: bold; color: #505050;">'.$school_name_user.'<br></p>
				';
