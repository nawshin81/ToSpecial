...
$get_info = mysqli_query($conn, "SELECT bio,queote,user_type FROM users WHERE username='$user'");
...
if ($updateinfo) {
	$bio = $_POST['bio'];
	$bio = trim($bio);
	$bio = ($bio);//mysql_real_escape_string
...
if ($update) {
	$queote = $_POST['queote'];
	$queote = trim($queote);
	$queote = ($queote);//mysql_real_escape_string
		//submit the form to database
		$info_submit_query = mysqli_query($conn, "UPDATE users SET queote='$queote' WHERE username='$user'");
		...
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					...
					<li style="border-bottom: none;"><a href="details_update.php" style="background-color: #e76f39; border-radius: 3px; color: #fff;">Details About</a></li>
