	...
	while ($row = mysqli_fetch_assoc ($password_query)) {
		...
		if ($oldpassword_md5 == $db_password) {
			if ($newpassword == $repear_password) {
				...
				$password_update_query = mysqli_query($conn, "UPDATE users SET password='$newpassword_md5', confirmCode='$confirmCode' WHERE username='$user'");
				$error = "<p class='succes_echo'>Success! Your password updated.</p>";
				// send email
				$msg = "".$db_first_name."
				
				Successfully your ToSpecial password has been changed. 
						
						If it's not by you, please follow the link.
				
						http://www.ToSpecial.com/confirmationPass.php?u=".$user."&code=".$confirmCode."
						
						";
						mail($db_email ,"ToSpecial Password Changed",$msg, "From:ToSpecial <no-reply@ToSpecial.com>");
				}
			}else {
		...
		<li style="float: left;">
			<div class="settingsleftcontent">
				<ul>
					...
					<li><a href="password_update.php" style=" background-color: #e76f39; border-radius: 3px; color: #fff;">Password</a></li>
