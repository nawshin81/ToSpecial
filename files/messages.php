...
//chat count 
$buddyList = '0';
$getonlineuser = mysqli_query($conn,"SELECT first_name,username FROM users WHERE (chatOnlineTime>=now()-300)") or die(mysql_error());

if (mysqli_num_rows($getonlineuser) >= '2' ) {
	while ($row = mysqli_fetch_assoc($getonlineuser)) {
		$usrnm= $row['username'];
		$check_if_friend = mysqli_query($conn,"SELECT * FROM follow WHERE (user_from='$usrnm' AND user_to='$user') || (user_from='$user' AND user_to='$usrnm') ORDER BY id DESC LIMIT 2");
...
<div style="width: 900px; margin: 52px auto;">
	<ul>
		<li style="float: left;">
			...
			<div>
				<h2 style='font-size: 20px';><a href="messages.php" style="text-decoration: none; color: #505050;">Message</a></h2><br />
			</div>
				<?php 
				//grab the message for the logged in user
				$grab_messages = mysqli_query($conn,"SELECT * FROM pvt_messages WHERE user_to='$user' OR user_from='$user' ORDER BY id DESC");
				...
				while ($get_msg = mysqli_fetch_assoc($grab_messages)) {
					...
					$get_user_info = mysqli_query($conn,"SELECT * FROM users WHERE username='$user_from'");
					...
					$get_userto_info = mysqli_query($conn,"SELECT * FROM users WHERE username='$user_to'");
		...
		if ($pro_changed_db != $profile_picuser_from_db) {
			...
			$profile_picuser_from = "userdata/profile_pics/".$profile_picuser_from_db;
					...
					//check for userto propic delete
						$pro_changed = mysqli_query($conn,"SELECT * FROM posts WHERE added_by='$user_to' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
			...
			$profile_picuser_to = "userdata/profile_pics/".$profile_picuser_to_db;
					...
					if (strlen($msg_body) > 50) {
						...
						if ($user_from == $user) {
							...
							echo "<div style='margin-left: 65px;'><b><a href='profile.php?u=$user_to' style='text-decoration: none; font-size: 14px; font-weight: bold; color: #505050;'>$msg_to_fname</a></b> <br>
							...
							echo "<div style='margin-left: 65px;'><b><a href='profile.php?u=$user_from' style='text-decoration: none; font-size: 14px; font-weight: bold; color: #505050;'>$msg_by_fname</a></b> <br>
		...
		<li style="float: right;">
			<div>
				...
				if (isset($_GET['u'])) {
					...
					if (ctype_alnum($username)) {
						//check user exists
						$check = mysqli_query($conn,"SELECT username, id,first_name FROM users WHERE username='$username'");
						if (mysqli_num_rows($check)===1) {
							$get = mysqli_fetch_assoc($check);
							$username = $get['username'];
							...
							if ($username != $user) {
								if (isset($_POST['submit'])) { 
									...
									$msg_body = ($msg_body);//mysql_real_escape_string
									$date = date("Y-m-d");
									$opened = "no";
									...
									$messages = mysqli_query($conn,"INSERT INTO pvt_messages VALUES ('','$user','$username','$msg_body','$date','','$opened')");
								...
								<h2 style='font-size: 20px';>To: $fullname</h2><br />";
								...
								$get_message = mysqli_query($conn,"SELECT * FROM pvt_messages WHERE (user_from='$user' AND user_to='$username') OR (user_from='$username' AND user_to='$user') ORDER BY id DESC LIMIT 50");
								$count = mysqli_num_rows($get_message);
								//deleting msg
								if ($count >= 51) {
									$result = mysqli_query($conn,"DELETE FROM pvt_messages WHERE (user_from='$user' AND user_to='$username') OR (user_from='$username' AND user_to='$user') ORDER BY id ASC LIMIT 1");
								}
								if ($count != 0) {
								while ($msg = mysqli_fetch_assoc($get_message)) {
									...
									//making emo
									$emoticonQuery = mysqli_query($conn,"SELECT * FROM emoticons");
						...
						$pro_changed = mysqli_query($conn,"SELECT * FROM posts WHERE added_by='$user_by ' AND (discription='changed his profile picture.' OR discription='changed her profile picture.') ORDER BY id DESC LIMIT 1");
						$get_pro_changed = mysqli_fetch_assoc($pro_changed);
		$pro_num = mysqli_num_rows($pro_changed);
		if ($pro_num == 0) {
			...
			$profile_pic = "userdata/profile_pics/".$profile_pic_db;
										...
										<div style='margin: 10px;'>
											...
											<div style='margin: 0 0 0 50px;'>
												<b><a href='profile.php?u=$user_by' style='text-decoration: none;  padding-left: 0px; font-size: 14px; color: #505050; font-weight: 700;' title=\"Go to $posted_by's Profile\" >$posted_by</a></b>
