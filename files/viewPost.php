...
if (isset($_REQUEST['pid'])) {
	...
	$id = ($_REQUEST['pid']);//mysql_real_escape_string
}else {
...
//for getting post
$getposts = mysqli_query($conn,"SELECT * FROM posts WHERE id ='$id'") or die(mysql_error());
...
if($sharepostid != 0) {
	...
	if($sp_added_by == $added_by) {
		if ($gender_user_db == '1') {
			$discription = "shared his <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: #505050;'>".$post_item."</a>";
		}else if ($gender_user_db == '2') {
			$discription = "shared her <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; color: #505050;'>".$post_item."</a>";
		}
	}else {
		$discription = "shared ".$sp_add_by."'s <a href='viewPost.php?pid=".$sharepostid."' style='text-decoration: none; padding-left: 0px; color: #505050;'>".$post_item."</a>";
	}
}
		...
		echo "<div style='min-height: 55px;' ><div style='float: left;'><img src='$profile_pic' style= 'border-radius: 22px'; title=\"$added_by\" height='45' width='45'  /></div>";
			...
			echo'<div class="posted_by">
				<span style="color: #9e9e9e;" >
					...
					echo'<a href="profile.php?u='.$added_by.'" style="text-decoration: none; color: #505050;">'.$add_by.'</a><span> '.$discription.'</span>';
					}else {
					echo'<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: #505050;">'.$add_by.'</a> > <a href="profile.php?u='.$user_posted_to.'" style="text-decoration: none; color: #505050;">'.$post_to_fname.'</a>';
					}
					...
					<div style="padding: 10px;" >';
						...
						<div style=" padding-left: 10px; overflow: hidden; ">
							<span style="color: #9197a3;" >
								<span style="font-weight: bold;">
									<a href="profile.php?u='.$sp_added_by.'" style="text-decoration: none; color: #505050;">'.$sp_add_by.'</a>
								</span>
							</span>
			...
			if ($rows_uname == 1) {
				echo "<a href='like.php?upid=".$id."' style='color: #505050;' >Liked . $count_like</a>";
			}else {
