...
$get_info = mysqli_query($conn,"SELECT first_name,nick_name,user_type FROM users WHERE username='$user'");
$get_row = mysqli_fetch_assoc($get_info);
$db_firstname = $get_row['first_name'];
$db_nickname = $get_row['nick_name'];
$type= $get_row['user_type'];

//submit what the user type in database
if ($updateinfo) {
	$firstname = strip_tags(@$_POST['fname']);
	$firstname = trim($firstname);
	$firstname = ($firstname);     //mysqli_real_escape_string
	$firstname = ucwords($firstname);

	if(strlen($firstname) >1 || strlen($firstname) < 16 )  {
		$info_submit_query = mysqli_query($conn,"UPDATE users SET first_name='$firstname' WHERE username='$user'");
		...
		<li style="float: left;">
			...
			<div class="settingsleftcontent">
				<ul>
					...
					<li><a href="account_update.php" style="background-color: #e76f39; border-radius: 3px; color: #fff;">Account</a></li>
			...
			<div class="settingsleftcontent">
				<p style="text-align: center; padding: 10px 0px;">ToSpecial</p>
			</div>
		</li>
