<?php 
	...
	if (isset($_POST['login'])) {
		if (isset($_POST['admin_user']) && isset($_POST['admin_pass'])) {
			$admin_user = ($_POST['admin_user']);	//mysql_real_escape_string
			$admin_pass = ($_POST['admin_pass']);	//mysql_real_escape_string
			$num = 0;
			$password_login_md5 = md5($admin_pass);
			$result = mysqli_query($conn,"SELECT * FROM admin_login WHERE user_name='$admin_user' AND user_pass='$admin_pass'");
